...
for (l in 1:2) {
  K = vec_K[l]
  q = vec_q[l]
  mat_pval = matrix(0, 4, num_trial)
  if (K == 2) {
    set.seed(2)
    for (j in 1:num_trial) {
      X = fun_gen_X(n, q, ss)
      mat_pval[1, j] = fun_proposed_exact(X, k1, k2)
    }
  } else {
    set.seed(2)
    alpha = 0.02
  ...
  for (j in 1:num_trial) {
    ...
    mat_pval[4, j] = test_hier_clusters_exact(X, "average", hc, K, k1, k2, 
                                              sig = sqrt(ss))$pval
  }
  list_mat[[l]] = mat_pval
}
...
to_save = "workspaces/sim_type1.RData"
